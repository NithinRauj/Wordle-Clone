...
const inputGuess = (key) => {
    ...
    if (isAlphabet(key)) {
        if (state.guesses[state.rowIndex].length !== 5) {
            ...
            document.getElementById(`tile_${state.rowIndex}${state.columnIndex}`).classList.remove('shake-tile-reverse');
        ...
        document.getElementById(`tile_${state.rowIndex}${state.columnIndex}`).classList.remove('shake-tile');
        document.getElementById(`tile_${state.rowIndex}${state.columnIndex}`).classList.add('shake-tile-reverse');
    }
}
...
const compareGuess = () => {
    ...
    state.canType = false;
    if (!dictionary.includes(guesses[rowIndex].join('').toLowerCase())) {
        document.getElementById('alert').classList.add('shake-alert');
        state.canType = true;
        return;
    }
